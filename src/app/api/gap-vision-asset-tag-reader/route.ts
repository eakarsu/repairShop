...
// AI gap: Vision-Based Asset Tag Reader (repairShop)
...
async function callLLM(systemPrompt: string, userMsg: string, maxTokens = 1400) {
  ...
  const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    method: 'POST',
    headers: { Authorization: 'Bearer ' + OPENROUTER_API_KEY, 'Content-Type': 'application/json', 'HTTP-Referer': 'http://localhost:3000', 'X-Title': 'repairShop Gap vision-asset-tag-reader' },
...
export async function POST(req: NextRequest) {
  try {
    ...
    const sys = "You parse serial numbers and asset tags from images and return structured data.";
    const user = `Body: ${JSON.stringify(body).slice(0, 4000)}`;
    const out = await callLLM(sys, user);
    gapFeatures.push({ at: new Date().toISOString(), slug: 'vision-asset-tag-reader', payload: { keys: Object.keys(body) } });
    return NextResponse.json({ reading: out });
  } catch (e: any) {
...
export async function GET() {
  return NextResponse.json({ feature: 'vision-asset-tag-reader', events: gapFeatures.filter(r => r.slug === 'vision-asset-tag-reader').length });
}
