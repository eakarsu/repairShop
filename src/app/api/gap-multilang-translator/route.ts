...
// AI gap: Multi-Language Customer Translator (repairShop)
...
async function callLLM(systemPrompt: string, userMsg: string, maxTokens = 1400) {
  ...
  const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    method: 'POST',
    headers: { Authorization: 'Bearer ' + OPENROUTER_API_KEY, 'Content-Type': 'application/json', 'HTTP-Referer': 'http://localhost:3000', 'X-Title': 'repairShop Gap multilang-translator' },
...
export async function POST(req: NextRequest) {
  try {
    ...
    const sys = "You translate customer-service messages while preserving repair-specific terminology.";
    const user = `Body: ${JSON.stringify(body).slice(0, 4000)}`;
    const out = await callLLM(sys, user);
    gapFeatures.push({ at: new Date().toISOString(), slug: 'multilang-translator', payload: { keys: Object.keys(body) } });
...
export async function GET() {
  return NextResponse.json({ feature: 'multilang-translator', events: gapFeatures.filter(r => r.slug === 'multilang-translator').length });
}
